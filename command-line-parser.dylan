...
define abstract open class <option> (<object>)
  ...
  // A function applied to the parsed option value. Example: store the value in a global
  // variable for easy access, `method (v) *my-flag* := v end`. This obviates the need
  // to lookup values by name in the parser.
  constant slot option-handler :: false-or(<function>) = #f,
...
define method parse-command-line
 ...
 => ()
  ...
  for (cmd in concatenate(list(parser), parser.parser-subcommands))
    for (opt in cmd.command-options)
      if (opt.option-handler)
        opt.option-handler(opt.option-value);
      end;
    end;
  end;
end method;
